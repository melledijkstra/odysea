...
describe('CliAuthFlowHandler', () => {
  ...
  it('should print the URL and return the pasted redirect URL', async () => {
    ...
    const mockQuestion = vi
      .fn()
      .mockResolvedValue('http://localhost:3000/callback?code=123&state=abc')
    ...
    expect(mockQuestion).toHaveBeenCalledWith(
      expect.stringContaining('Paste the redirect URL here')
    )
    expect(mockClose).toHaveBeenCalled()
    expect(result.href).toBe(
      'http://localhost:3000/callback?code=123&state=abc'
    )
  })
})
